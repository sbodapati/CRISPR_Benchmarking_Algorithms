...
---
  ...
  ```{r}
testCounts = read.table(file = "/Users/sbodapati/Documents/Qi Lab/Benchmarking/finalSimData.csv", header = FALSE, sep = ",")
head(testCounts)
counts = testCounts[ ,1:4]
...
library(ggplot2)
ggplot(data.frame(log2fc = log2fc, essential = factor(testCounts[ ,5])), aes(x = log2fc, col = essential, fill = essential)) + geom_density(alpha = 0.6) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))

testCounts = read.table(file = "/Users/sbodapati/Documents/Qi Lab/Benchmarking/TKO.txt", header = TRUE)
head(testCounts)
